...
function drawFibLines(d, width) {
  // Supported levels and colors
  const levels = [
    { p: 0, color: "#e53935" },
...
export default function DrawingOverlay({ alerts }) {
  ...
  // Render alert markers/lines if you pass an "alerts" prop (array of {value: price, ...})
  // Example: alerts?.map(({ value }, idx) => (<...>))
  ...
  return (
    <svg
      ...
      // onMouseMove, onMouseUp handlers as previously coded
    >
      {/* Draw alert horizontal markers here if alerts provided */}
      {alerts &&
        alerts.map((a, idx) => (
          <g key={`alert-${a.id || idx}`}>
            <line
              x1={0}
              y1={a.y} // You must supply chart-pixel y value in the parent, or derive from chart scale here
              x2={2000}
              y2={a.y}
              stroke="#fbc02d"
              strokeDasharray="4"
        ...
        d.type === "trendline" ? (
          ...
          <g key={i} onClick={() => setSelectedId(i)}>
            <rect
              ...
              stroke={i === selectedId ? "#2962ff" : "#2962ff"}
              strokeWidth={i === selectedId ? 3.5 : 2}
